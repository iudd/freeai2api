...
// @version      1.2.1
// @description  Extract API information from mindvideo.ai/zh for curl usage with improved loading
// @author       iudd
...
(function() {
    ...
    function extractPageInfo() {
        try {
            ...
            addDebugInfo(`页面信息提取出错: ${error.message}`);
            return {
                website: 'MindVideo',
                url: window.location.href,
                title: document.title,
        ...
        try {
            ...
            window.fetch = function(...args) {
                try {
                    ...
                    addDebugInfo(`Fetch拦截错误: ${error.message}`);
                    return originalFetch.apply(this, args);
                }
            };
            ...
            window.XMLHttpRequest = function() {
                try {
                    ...
                    addDebugInfo(`XHR拦截错误: ${error.message}`);
                    return new originalXMLHttpRequest();
                }
            };
        } catch (error) {
            addDebugInfo(`请求拦截初始化错误: ${error.message}`);
        }
    }
    ...
    function stopInterceptingRequests() {
        try {
            ...
            addDebugInfo(`停止拦截错误: ${error.message}`);
        }
    }
    ...
    function generateCurlCommand(request) {
        try {
            ...
            return `curl -X ${request.method} "${request.url}"  # 生成错误: ${error.message}`;
        }
    }
    ...
    function copyToClipboard(text) {
        try {
            ...
            addDebugInfo(`复制错误: ${error.message}`);
        }
    }
    ...
    function manualTrigger() {
        try {
            ...
            addDebugInfo(`手动触发错误: ${error.message}`);
        }
    }
        ...
        try {
            ...
            let html = `
                ...
                <div class="api-info">
                    ...
                    ${pageInfo.error ? `<p><strong>错误:</strong> <span style="color: #ff6b6b;">${pageInfo.error}</span></p>` : ''}
                </div>
            ...
            addDebugInfo(`面板更新错误: ${error.message}`);
        }
    }
        ...
        try {
            ...
            panel.innerHTML = `
                <div class="panel-header">
                    🎯 MindVideo API提取器 v1.2.1
                    <span style="font-size: 11px; color: #ccc;">修复加载问题</span>
                </div>
            ...
            // 立即开始初始化
            addDebugInfo('面板创建完成，开始初始化...');
            startInterceptingRequests();
            updatePanel();
        } catch (error) {
            console.error('创建面板错误:', error);
        }
    }
    ...
    function listenForCreateButton() {
        try {
            ...
            addDebugInfo(`按钮监听初始化错误: ${error.message}`);
        }
        
        // 每隔2秒重新检查按钮
        setTimeout(listenForCreateButton, 2000);
    }
    ...
    function createToggleButton() {
        try {
            ...
            addDebugInfo('浮动按钮创建完成');
        } catch (error) {
            console.error('创建浮动按钮错误:', error);
        }
    }

    // 初始化
    try {
        addDebugInfo('MindVideo API提取器 v1.2.1 已加载 - 修复加载问题');
        createToggleButton();
