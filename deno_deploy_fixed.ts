...
/**
 * Deno Deploy 兼容版本 - 修复了 Deno.exit() 问题
 * 
 * 这是 openai_adapter.ts 的修复版本，专门为 Deno Deploy 优化
 */
...
async function handleOpenAIRequest(request: Request): Promise<Response> {
  ...
  try {
    ...
    if (path === '/generate' && request.method === 'POST') {
      ...
      if (!prompt) {
        return new Response(JSON.stringify(createOpenAIErrorResponse("Missing prompt parameter")), {
          status: 400,
          headers: { ...headers, 'Content-Type': 'application/json' }
        });
      }
...
async function startServer() {
  console.log('🚀 FreeAI2API Deno Deploy 兼容版本启动中...');
  ...
  try {
    ...
    // Deno Deploy环境启动
    await serve(handler, {
      port: 8080, // Deno Deploy默认端口
      hostname: "0.0.0.0"
    });
    
  } catch (error) {
    console.error('❌ 启动失败:', error.message);
    // 在Deno Deploy环境中，我们抛出错误而不是调用Deno.exit()
    throw error;
  }
}
